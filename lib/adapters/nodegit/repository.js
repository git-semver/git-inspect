...
class RepositoryAdapter
{
  ...
  /**
   * Return true if topic branch outdated about trunk branches
   *
   * @param {Branch}
   * @param {Branch[]}
   * @return {boolean}
   */
  async isOutdated(branch, branches)
  {
    const topicBranchRef = branch.source;
    const topicHeadCommit = await this.repo.getBranchCommit(topicBranchRef);

    console.log('Topic', branch.name);

    for (let i in branches)
    {
      const trunkBranchRef = branches[i].source;

      if (topicBranchRef.name() === trunkBranchRef.name())
      {
        continue;
      }

      const trunkHeadCommit = await this.repo.getBranchCommit(trunkBranchRef);

      console.log('Trunk', branches[i].name);

      const uniqTrunkCommits = await this.git.Graph.aheadBehind(this.repo, trunkHeadCommit, topicHeadCommit);
      const uniqTopicCommits = await this.git.Graph.aheadBehind(this.repo, topicHeadCommit, trunkHeadCommit);

      console.log('uniqTrunkCommits', uniqTrunkCommits);
      console.log('uniqTopicCommits', uniqTopicCommits);
      /*if (topicHeadCommit.date() > trunkHeadCommit.date())
      {
