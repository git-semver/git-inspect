...
class LinearBranchReducer
{
  ...
  async getGraphInfo(branch)
  {
    ...
    for (let i = 0; i < commits.length - 1; i++)
    {
      ...
      const isDesc = await this.adapter.isDescendantOf(cur, anc);
    ...
    const linearFactor = (commits.length - cousins.length) / commits.length;

    return {
      cousins,
      linearFactor: Math.floor(linearFactor * 100) / 100,
  ...
  afterReduce(linear)
  {
    ...
    linear.linearFactor = Math.floor(factor * 100) / 100;
