...
class Report
{
  ...
  fillRepository({ remote, directoryPath })
  {
    this.inst.repository.remote = remote;
    this.inst.repository.directoryPath = directoryPath;
    return this;
  }
  ...
  /**
   * Get mapped report results
   *
   * Return mapped results field report
   *
   * ```
   * {
   *   "<inspector>": {
  ...
  {
    const mapped = {};
    const { results } = this.inst;

    for (let i in results)
    {
      const { inspector, reducer, report } = results[i];
      if (mapped[inspector] === undefined)
      {
        mapped[inspector] = {};
      }
      mapped[inspector][reducer] = report;
    }
